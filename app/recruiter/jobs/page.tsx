...
export default function JobsPage() {
  ...
  const copy = async (job: Job) => { await navigator.clipboard.writeText(`${window.location.origin}/interview?job=${encodeURIComponent(job.id)}`); setCopied(job.id); setTimeout(() => setCopied(""), 1800); };
